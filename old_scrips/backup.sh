...
#===============================================================================
# Basic settings ---------------------------------------------------------------
BACKUPHOME=/mnt/Backup/@incre_back          # Home directory of backup
...
LOG_DIR_B=$BACKUPHOME/log                   # Directory for log
...
$snDel -s $SNAP_PATH -d weeks -n 2 >$LOG_DIR_C/snap_del.log

# Delect log more then 14 days
find $LOG_DIR_B/* -maxdepth 0 -type d -ctime +13 \
...
for DIR in ${DIR_LIST[@]}; do
    ...
    rsync \
        ...
        --exclude-from=$BACKUPHOME/filter/exclude_files-$DIR.txt \
        --log-file=$LOG_DIR_C/$DIR-Back.log \
        /mnt/$DIR/ $BACKUPHOME/current/$DIR/
done
